...
export default function InsumoForm({formFields, setFormValues, formValues, onSubmitHandler, formErrors}) {
    ...
    return (
        <div className="pl-10 pr-10">
            <h1 className="text-3xl font-semibold mb-8">Crear Insumo</h1>
            <form onSubmit={(e) => formSubmit(e)}>
                {
                    errors.length > 0 && (
                    <div className="flex flex-col rounded-md py-3 px-3 mb-5 bg-red-600">
                        <ul>
                            { errors.map((error, index) => <li key={randomID} className="text-white">{error.message}</li> )}
                ...
                <div className="flex justify-end">
                    <button className="bg-purple-600 text-white rounded-lg font-bold hover:bg-yellow-400 p-2 ml-2 flex items-center transition-all 1s">
                        <span>Guardar</span>
                    </button>
                </div>
            </form>
